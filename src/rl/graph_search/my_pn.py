...
class GuidedPolicy(nn.Module):
    def __init__(self, args):
        ...
        self.fn = None
        self.fn_kg = None
        self.inner_path_trace = None

        #experts
        self.relation2experts = {}
        self.dummy_expert = None
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        X2 = self.W2Dropout(X)
        #[2560,400]
        def policy_nn_fun(X2, action_space, trace, expert):
            '''
            trace_r, trace_e (bucket_batch_size, trace_length)
            todo: we could only put the trace which is not masked into calculation to save memory
            X2 (bucket_batch_size, action_embedding)
            r_space, e_space, action_mask (bucket_batch_size, action_space_size)
            A (bucket_batch_size, action_space_size, action_embedding)
            A@X2 (bucket_batch_size, action_space_size)
            '''
            (r_space, e_space), action_mask = action_space
            experts, refs = expert
            bucket_batch_size = r_space.shape[0]
            #todo: add attention module here
            #LSTM part
            A = self.get_action_embedding((r_space, e_space), kg)
            action_dist = torch.squeeze(A @ torch.unsqueeze(X2, 2), 2) - (1 - action_mask) * ops.HUGE_INT
            #attn part
            if len(experts) == 0:
                bias = var_cuda(torch.zeros_like(action_dist), requires_grad=False)
            else:
                trace_r, trace_e = trace
                trace_r = trace_r[refs, :]
                trace_e = trace_e[refs, :]
                action_space_size = r_space.shape[-1]
                trace_r = trace_r.unsqueeze(1)
                trace_r = trace_r.expand([trace_r.shape[0], action_space_size, trace_r.shape[-1]])
                trace_e = trace_e.unsqueeze(1)
                trace_e = trace_e.expand([trace_e.shape[0], action_space_size, trace_e.shape[-1]])
                new_trace_r = torch.concat([trace_r,r_space[refs].unsqueeze(-1)], dim=-1)
                new_trace_e = torch.concat([trace_e,e_space[refs].unsqueeze(-1)], dim=-1)
                trace_action = self.get_action_embedding((new_trace_r,new_trace_e), kg)
                action_seq = trace_action.shape[2]
                V = torch.matmul(
                    trace_action.unsqueeze(2).unsqueeze(-2),
                    experts[:,:,:action_seq,:].unsqueeze(1).unsqueeze(-1)
                ).squeeze(-1).squeeze(-1)
                Q = self.WQ(experts)
                K = self.WK(trace_action)
                alpha = torch.matmul(
                    Q.unsqueeze(1),
                    K.unsqueeze(2).transpose(-1,-2)
                )
                alpha = alpha / math.log(self.action_dim)
                alpha = F.softmax(alpha, dim = -1)
                bias = torch.matmul(
                    alpha,
                    V.unsqueeze(-1)
                ).squeeze(-1)
                #todo remove appendix calculation on masked cases
                bias = torch.mean(torch.mean(bias, dim = -1), dim = -1)
                bias = F.pad(bias, (0, 0, 0, bucket_batch_size - bias.shape[0]), mode="constant", value=0)
                rearrange = [-1] * bucket_batch_size
                for i, ref in enumerate(refs):
                    rearrange[ref] = i
                bias = bias[rearrange, :]

            action_dist = F.softmax(action_dist + bias, dim = -1)
        ...
        if use_action_space_bucketing:
            ...
            #db_references is not a tensor, elements of db_action_spaces are\
            trace_r = torch.concat([action[0].unsqueeze(1) for action in self.inner_path_trace],dim=1)
            trace_e = torch.concat([action[1].unsqueeze(1) for action in self.inner_path_trace],dim=1)
            for action_space_b, reference_b in zip(db_action_spaces, db_references):
                X2_b = X2[reference_b, :]
                trace_rb = trace_r[reference_b, :]
                trace_eb = trace_e[reference_b, :]
                experts, refs = self.sample_experts(q[reference_b],kg)
                action_dist_b, entropy_b = policy_nn_fun(X2_b, action_space_b, (trace_rb, trace_eb), (experts, refs))
    ...
    def initialize_path(self, init_action, kg):
        ...
        if self.dummy_expert is None:
            self.init_dummy_expert(kg)
        ...
        self.inner_path_trace = [init_action]
    ...
    def update_path(self, action, kg, offset=None):
        ...
        :param KG: Knowledge graph environment.
        """
        def offset_path_history(p, trace, offset):
        ...
        if offset is not None:
            offset_path_history(self.path, self.inner_path_trace, offset)

        self.path.append(self.path_encoder(action_embedding.unsqueeze(1), self.path[-1])[1])
        self.inner_path_trace.append(action)

    def load_experts(self, experts):
        #todo: check whether the format is right
        self.relation2experts = experts

    def update_experts(self, relations, traces, experts):
        for i, ex in enumerate(experts):
            if ex:
                relation = relations[i]
                path = [(r[i], e[i]) for (r, e) in traces]
                if relation not in self.relation2experts:
                    self.relation2experts[relation] = []
                self.relation2experts[relation].append(path)

    def init_dummy_expert(self, kg):
        dummy_expert_r = [kg.dummy_r] * (self.args.num_rollout_steps + 1)
        dummy_expert_r = [dummy_expert_r] * self.args.num_expert
        dummy_expert_e = [kg.dummy_e] * (self.args.num_rollout_steps + 1)
        dummy_expert_e = [dummy_expert_e] * self.args.num_expert
        self.dummy_expert = (torch.tensor(dummy_expert_r), torch.tensor(dummy_expert_e))

    def sample_experts(self, relations, kg):
        refs = []
        expert_samples = []
        for i,r in enumerate(relations):
            r = r.item()
            if (r not in self.relation2experts) or (len(self.relation2experts[r]) < self.args.num_expert):
                continue
            else:
                expert = sample(self.relation2experts[r], k = self.args.num_expert)
                expert_r = [[r for (r, e) in p] for p in expert]
                expert_e = [[e for (r, e) in p] for p in expert]
                expert = (torch.tensor(expert_r), torch.tensor(expert_e))
                expert_samples.append(expert)
                refs.append(i)
        if len(expert_samples) > 0:
            expert_samples = (var_cuda(torch.concat([e[0].unsqueeze(0) for e in expert_samples], dim = 0), requires_grad=False), 
                    var_cuda(torch.concat([e[1].unsqueeze(0) for e in expert_samples], dim = 0), requires_grad=False)
                )
            expert_samples = self.get_action_embedding(expert_samples, kg)
        return expert_samples, refs
    ...
    def get_action_space_in_buckets(self, e, obs, kg, collapse_entities=False):
        ...
        if collapse_entities:
            ...
            for key in batch_ref:
                action_space = kg.action_space_buckets[key] #((r_sapce,e_space),mask)
    ...
    def define_modules(self):
        ...
        if self.relation_only_in_path:
            ...
            self.path_encoder = nn.LSTM(input_size=self.action_dim,
                                        ...
                                        num_layers=self.history_num_layers,
                                        batch_first=True)
        self.WQ = nn.Linear(self.action_dim, self.action_dim)
        self.WK = nn.Linear(self.action_dim, self.action_dim)
